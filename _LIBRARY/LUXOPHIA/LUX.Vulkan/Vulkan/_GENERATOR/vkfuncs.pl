...
o('// （TVkDevices がインスタンス生成直後に自動で呼ぶ。取得される入口はローダの');
o('//   トランポリンなので、デバイスが何個あっても正しく振り分けられる）');
o('procedure LoadInstanceFunctions( const instance_:T_VkInstance );');
o('');
o('// vkGetDeviceProcAddr 経由で、デバイス階層の入口を取得し直す。');
o('// ローダのトランポリンを介さなくなるので、呼び出しが速くなるだけの最適化。');
o('// ※ 取得した入口は指定デバイス専用。関数ポインタはグローバル変数なので、');
o('//    VkDevice を複数使うアプリで呼ぶと他デバイスの呼び出しが壊れる。');
o('//    単一デバイスと分かっているアプリだけが任意で呼ぶこと（自動では呼ばれない）。');
